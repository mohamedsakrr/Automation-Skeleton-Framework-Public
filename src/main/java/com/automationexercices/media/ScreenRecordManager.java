...
public class ScreenRecordManager {
    ...
    public static void stopRecording(String testMethodName) {
        stopRecording(testMethodName, true);
    }

    public static File stopRecording(String testMethodName, boolean keepRecording) {
        try {
            if (recorder.get() != null) {
                ...
                if (!keepRecording) {
                    deleteRecording(videoFile);
                    LogsManager.info("Recording discarded for passed test: " + testMethodName);
                    return null;
                }
    ...
    private static void deleteRecording(File recording) {
        if (recording != null && recording.exists() && !recording.delete()) {
            LogsManager.warn("Failed to delete recording: " + recording.getAbsolutePath());
        }
    }
